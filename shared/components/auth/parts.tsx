...
export function WhatsAppButton({ onVerified }: { onVerified?: (accessToken?: string) => void | Promise<void> }) {
  ...
  async function verify(){if(!validOtp(otp)){setError("Enter the six-digit code.");return}setBusy(true);setError("");const supabase=createAuthBrowserClient();const {data,error}=await supabase.auth.verifyOtp({phone:e164,token:otp,type:"sms"});let accessToken=data.session?.access_token;if(!error&&!accessToken){const refreshed=await supabase.auth.getSession();accessToken=refreshed.data.session?.access_token}setBusy(false);if(error||!accessToken){setError("The code is incorrect or expired. Request a new code and try again.");return}await onVerified?.(accessToken)}
  return (
